...
simparam4 = "_mu_v_%.1f" % (Mu_v)
simparam5 = "_tau_%.5f" % (Tau)
simparam6 = "_dt_%.5f" % (dt)
...
# Save the Results
results = XDMFFile(savedir + "/" + name + simparam1 + simparam2 + simparam3 + simparam4 + simparam5 + simparam6 + ".xdmf")
...
# Isochoric Quantities
Fstag_bar = (Jstag**(-1/3)) * Fstag
Cstag_bar = Fstag_bar.T*Fstag_bar
...
for i, inc in enumerate(ind_steps):
    ...
    ATensor.rename("Internal Variable", "A")
...
plt.tight_layout()
plt.savefig(savedir + "/" + "Traction_vs_Displacement" + simparam1 + simparam2 + simparam3 + simparam4 + simparam5 + simparam6 + ".png", dpi=300)
